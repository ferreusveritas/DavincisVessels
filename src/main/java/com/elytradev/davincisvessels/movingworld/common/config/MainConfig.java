package com.elytradev.davincisvessels.movingworld.common.config;
...
import com.elytradev.davincisvessels.movingworld.common.config.priority.AssemblePriorityConfig;
...
public class MainConfig {
    ...
    public void postLoad() {
        ...
        for (int i = 0; i < shared.loadedBlockDensities.length; i++) {
            ...
            if (pair.length != 2) {
            	DavincisVesselsMod.LOG.warn("Invalid key-value pair at block_densities[" + i + "]");
                continue;
            }
            String key = pair[0];
            float density;
            try {
            	...
            	DavincisVesselsMod.LOG.warn("Cannot parse value " + pair[1] + " to floating point at block_densities[" + i + "]");
                continue;
            }
            Block block = Block.getBlockFromName(key);
            if (block == null) {
            	DavincisVesselsMod.LOG.warn("No block found for " + key + " at block_densities[" + i + "]");
                continue;
            }
        ...
        for (int i = 0; i < shared.loadedMaterialDensities.length; i++) {
            ...
            String[] pair = splitpattern.split(s);
            if (pair.length != 2) {
            	DavincisVesselsMod.LOG.warn("Invalid key-value pair at material_densities[" + i + "]");
                continue;
            }
            String key = pair[0];
            float density;
            try {
            	...
            	DavincisVesselsMod.LOG.warn("Cannot parse value " + pair[1] + " to floating point at material_densities[" + i + "]");
                continue;
            }
            Block block = Block.getBlockFromName(key);
            if (block == null) {
            	DavincisVesselsMod.LOG.warn("No block found for " + key + " at material_densities[" + i + "]");
                continue;
            }
