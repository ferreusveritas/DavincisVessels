package com.elytradev.davincisvessels.movingworld.common.chunk.assembly;
...
import com.elytradev.davincisvessels.movingworld.common.event.DisassembleBlockEvent;
...
import com.elytradev.davincisvessels.movingworld.common.util.MathHelperMod;
import com.elytradev.davincisvessels.movingworld.common.util.RotationHelper;
...
public class ChunkDisassembler {
	...
	public boolean canDisassemble(MovingWorldAssemblyInteractor assemblyInteractor) {
		...
		ZoneManager manager = ZoneManager.get(world);
			...
			for (int j = chunk.minY(); j < chunk.maxY(); j++) {
				for (int k = chunk.minZ(); k < chunk.maxZ(); k++) {
					...
					if ((block != null && !block.isAir(state, world, pos) && !block.getMaterial(state).isLiquid() && !assemblyInteractor.canOverwriteState(state))
							|| (MathHelperMod.round_double(vec.y + movingWorld.posY) > world.getActualHeight()) || manager.testPlaceBounds(pos)) {
						return false;
					}
				}
			}
	...
	public AssembleResult doDisassemble(MovingWorldAssemblyInteractor assemblyInteractor) {
		...
		tileMarker = null;
		if (movingWorld.getMobileChunk().marker != null && movingWorld.getMobileChunk().marker.tileEntity instanceof TileMovingMarkingBlock) {
		...
		try {
			...
			DavincisVesselsMod.LOG.error("Exception while disassembling, reverting doTileDrops... ", exception);
		...
		for (LocatedBlockList pList : sortedPostList) {
			if (pList != null && !pList.isEmpty())
				for (LocatedBlock locatedBlockInstance : pList) {
					...
					DavincisVesselsMod.LOG.debug("Post-rejoining block: " + locatedBlockInstance.toString());
