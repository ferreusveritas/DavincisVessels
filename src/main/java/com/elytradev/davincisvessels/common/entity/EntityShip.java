...
import com.elytradev.davincisvessels.movingworld.common.entity.MovingWorldHandlerCommon;
	...
	public boolean disassemble(boolean overwrite) {
		...
		if (!disassembler.canDisassemble(getNewAssemblyInteractor())) {
			if (prevRiddenByEntity instanceof EntityPlayer) {
				...
				((EntityPlayer) prevRiddenByEntity).sendStatusMessage(testMessage, false);
			}
			return false;
		}
